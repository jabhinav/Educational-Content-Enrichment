...
# execfile("def_extract.py")
exec(compile(open('def_extract.py', 'rb').read(), 'def_extract.py', 'exec'))

with open ('def_in_text', 'rb') as fp:
	def_in_text = pickle.load(fp)

with open ('def_in_wiki', 'rb') as fp:
	def_in_wiki = pickle.load(fp)

with open ('definitions', 'rb') as fp:
...
if def_in_text == True:
	...
	for ix in sent_text:
		for iy in definitions:
			if ix == iy:
				final_highlighted_text += '<span style="background-color: #FFFF00">'+ix+'</span>'
			else:
				final_highlighted_text += '<span style="background-color: #FFFFFF">'+ix+'</span>'

	with open('def','wb') as fp1:
		pickle.dump(final_highlighted_text, fp1)
	with open('def_check','wb') as fp1:
		pickle.dump("True", fp1)

elif def_in_text == False and def_in_wiki == True:
	for ix in definitions:
		def_text += ix

	with open('def','wb') as fp1:
		pickle.dump(def_text, fp1)
	with open('def_check','wb') as fp1:
		pickle.dump("False", fp1)
...
with open('templates/results1.html', 'a') as table_file:
